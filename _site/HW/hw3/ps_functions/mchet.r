...
# Filename of pdf
pdffile <- "hetScen1.pdf"

# Main title of pdf
maintitle <- "LS with heteroskedasticity"
...
# NOTE ADDITION:  parameters of the function for sigma
g <- c(log(2),0)
...
for (j in 1:sims) {
  ...
  os <- rep(1,n)
  mu <- cbind(os,X)%*%b
  # NOTE ADDITION: calculation of sigma
  # the exp() makes sure the variance is always positive
  sigma <- sqrt(exp(cbind(os,X[,1])%*%g))
  y <- mu + rnorm(n)*sigma
