...
# Filename of pdf
pdffile <- "selectScen1.pdf"

# Main title of pdf
maintitle <- "Selection on Dependent Variable"
...
for (j in 1:sims) {
  ...
  # NOTE CHANGE:  we delete all observations for which y is greater than
  # its sample mean value
  selectX <- X[y<mean(y),]
  selecty <- y[y<mean(y)]
  res <- lm(selecty~selectX)
