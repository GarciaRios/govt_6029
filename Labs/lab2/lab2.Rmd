...
knitr::opts_chunk$set(results = "hide", warning = FALSE, message = FALSE, echo = T, eval = F,
                      fig.show = "hide")
```
...
For this lab we will use the following libraries `dplyr` and `ggplot2`. We can load those individually or use `tidyverse` which is not really a package but a collection of packages. Lately, I only load `tidyverse` since it contains all the packages that I frequently use. Rmember, make sure you have installed these packages before you loand load them.
...
You can see which datasets are included in a package. gapminder is not the only one most packages have some data set within the package. See also for instance `ggplot2`
```{r eval=FALSE}
...
- We selced only `year` and `lifeExp` columns, _then_
...
There are are several ways to merge datasets with **dplyr** (left join, right join, inner join, and outer join which differ in which observations it matches and keep).
...
No we can take `gdpPercap` and divide it by  `usa_gdpPercap to obtrain a relative to the US figure.
...
For the most part, to vizualize results in this course we will be using the graphics package **ggplot2**, which is one of the most popular, but it is only one of several graphics packages in R.[^1]
...
This just initializes the plot with  the basic mapping. We still need to tell `ggplot` the geometric object (or `geoms`) that we will to repsent the data in this mapping.
...
That looks okay but it would probably look be better if we log transform.  Notice that we don't have to create a new variable we can just  do the transformation inside the aestethics.
...
Note the common workflow: gradually build up the plot you want,  re-define the plot by adding (litterally) new elemets.


Now, let's vary add another variable by having those points be reprebnted by different col continent variable to aesthetic color according to the continent they belong. 
...
Add now a fitted curve or line, let's forget about continent for now
...
Let's remove the confidece band
...
The dafult of `geom_smooth` follows a  local polynomial tegression fitting (aka LOESS) but we can force any toher type of fitting. Let's try a linear model `lm`
...
That's a lot of info. Let's try something else called facceting. Thiscreates various panels or subplots by a given variable.
```{r}
ggplot(gapminder, aes(x = gdpPercap, y = lifeExp, color = continent)) + 
  geom_point() + 
  scale_x_log10() + 
...
Great! So we need to account for time. Let's use some of the cool capabilites of the extentions in the `ggplot`verse with the `gganimate` package
...
Ok, back to the anbalysis. What if I am only interrested in the US?
```{r}
